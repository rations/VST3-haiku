...
//
// Category    : Helpers
// Filename    : public.sdk/source/vst/hosting/module_haiku.cpp
// Created by  : Steinberg, 08/2016
// Description : hosting module classes (Haiku implementation)
//
//-----------------------------------------------------------------------------
...
{
	// Haiku has no /proc/<pid>/exe; ask the kernel for the team's app image.
	image_info info;
	int32 cookie = 0;
...
//------------------------------------------------------------------------
class HaikuModule : public Module
{
public:
	template <typename T>
	...
	static Optional<Path> getSOPath (const std::string& inPath)
	{
		...
		// use the Machine Hardware Name (from uname cmd-line) as prefix for "-haiku"
		auto machine = getCurrentMachineName ();
		if (!machine)
			return {};

		modulePath /= *machine + "-haiku";
		if (!filesystem::is_directory (modulePath))
			return {};
...
{
	auto _module = std::make_shared<HaikuModule> ();
...
bool Module::validateBundleStructure (const std::string& modulePath, std::string& errorDescription)
{
	...
	if (filesystem::exists (path) == false)
	{
		errorDescription = "Expecting '" + *machine + "-haiku' as architecture subfolder.";
		return false;
	}
